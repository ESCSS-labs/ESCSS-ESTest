    ...
    try {
      if (process.env.NODE_ENV === 'production') throw new Error(`\n 📝 Public Message: ${pubMsg} \n 🚫 Details hidden for security. Check in dev mode.`)
        ...
        if (typeof pubMsg !== 'string') {
          setTimeout(() => console.log(`❌ Received: '${getTokenType(pubMsg)}'`, pubMsg), 0)
        ...
        // Invalid 2nd argument
          ...
          throw new Error(`\n 📝 Public Message: ${pubMsg} \n ✅ Expected 2nd Argument (tokenType): 'Undefined' | 'Null' | 'Array' | 'Date' | 'Object' | 'Boolean' | 'NaN' | 'Number' | 'BigInt' | 'String' | 'Symbol' | 'Function'  | 'RegExp'`)
        }
